function  [modelParameters,firingData] = positionEstimatorTraining_Ciarra(trainingData)
...
noDirections = 8;
group = 20; % bin size
win = 50; % window size for Gaussian Kernel Smoothing
...
for trimmer = timePoints
    ...
    % need to get (neurons x time)x trial
    for i = 1: noDirections
        for j = 1: noTrain %number of trials
            for k = 1: trimmer %bin number
                firingData(noNeurons*(k-1)+1 : noNeurons*k , noTrain*(i-1)+j) = trialFinal(j,i).rates(:,k);     
            end
        end
    end
